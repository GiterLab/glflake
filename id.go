...
func (j JSONSyntaxError) Error() string {
	return fmt.Sprintf("invalid glflake ID %q", string(j.original))
}
...
// An ID is a custom type used for a glflake ID.  This is used so we can
...
func (f ID) LeadingZerosString(zeroN uint8) string {
	format := "%0" + strconv.Itoa(int(zeroN)) + "d"
...
// ParseBase32 parses a base32 []byte into a glflake ID
// NOTE: There are many different base32 implementations so becareful when
// doing any interoperation.
...
// IntBytes returns an array of bytes of the glflake ID, encoded as a
// big endian integer.
